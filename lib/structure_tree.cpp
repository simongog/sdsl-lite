...
void structure_tree_node::add_size(uint64_t value) {
    m_key_values[STRUCTURE_TREE_SIZE_KEY] = util::to_string(value);
}
...
void structure_tree_node::rec_merge(const structure_tree_node &v){
	if ( m_key_values.find(STRUCTURE_TREE_SIZE_KEY) != m_key_values.end() ){
		...
		m_key_values[STRUCTURE_TREE_SIZE_KEY] = util::to_string(size + vsize);
	}
}
...
bool structure_tree::merge_children(structure_tree_node *v){
	if ( NULL != v and v->m_children.size() > 0 ){
		...
		if( success ){
			...
			v->key_values[STRUCTURE_TREE_NAME_KEY] = "[0.." + util::to_string(m_1) + "]";
		}
	}
	return true;
}
