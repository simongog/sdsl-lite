...
osfstream::osfstream(const std::string& file, std::ios_base::openmode mode) :
    m_streambuf(NULL), m_file(""), m_use_ram(true)
{
    this->init(m_streambuf);
    open(file, mode);
}
...
osfstream::open(const std::string& file, std::ios_base::openmode mode)
{
    ...
    if (m_use_ram) {
        std::cerr<<"OSFSTREAM: try to open stringbuf" <<std::endl;
//            m_streambuf = new std::stringbuf(ram_fs::content(m_file), mode | std::ios_base::out);
        m_streambuf = new std::stringbuf(ram_fs::content(m_file), std::ios_base::out);
        std::cerr<<"m_stringbuf="<<(size_t)m_streambuf<<std::endl;
    } else {
        ...
        if (!f_buf->open(m_file.c_str(), mode | std::fstream::out)) {
            ...
            f_buf = NULL;
            std::cerr<<"ERROR OSFSTREAM: could not open `"<<m_file<<"`"<<std::endl;
        } else {
            this->clear();
        }
        m_streambuf = f_buf;
...
osfstream::close()
{
    ...
    if (m_use_ram and !m_closed) {
        ...
        if (((std::filebuf*)m_streambuf)->close()) {
            ...
            std::cerr<<"STREAM: set failbit"<<std::endl;
            this->setstate(std::ios::failbit);
        }
    }
}
...
isfstream::isfstream() : m_streambuf(NULL), m_file(""), m_use_ram(true)
{
    this->init(m_streambuf);
}
