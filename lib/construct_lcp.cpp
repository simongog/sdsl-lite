...
// //			load_from_file(lcp_sml2, (dir+"lcp_sml_go").c_str() );
...
// alphabet[sigma] = '\0';
// {
// std::ofstream lcp_sml_out_buf(cache_file_name("lcp_sml", config).c_str(), std::ios::binary | std::ios::trunc | std::ios::out);    // open out file stream
...
// register_cache_file("lcp_sml", config);
// }
// util::clear(text);
// if (n > 1000 and nn > 5*(n/6)) {  // if we would occupy more space than the PHI algorithm => switch to PHI algorithm
// construct_lcp_PHI<8>(config);
