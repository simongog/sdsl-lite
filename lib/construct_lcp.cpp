...
//                    std::cerr<<"n="<<n<<" "<<" plcp.size()="<<plcp.size();
   ...
   std::ofstream lcp_out_buf(util::cache_file_name(constants::KEY_LCP, config).c_str(), std::ios::binary | std::ios::app | std::ios::out);   // open buffer for plcp
...
void construct_lcp_simple_5n(cache_config& config)
{
   ...
   int_vector_file_buffer<> sa_buf(util::cache_file_name(constants::KEY_SA, config));   // initialize buffer for suffix array
   sa_buf.load_next_block();
   size_type sai_1 = sa_buf[0];  // store value of sa[i-1]
   int_vector_file_buffer<8> bwt_buf(util::cache_file_name(constants::KEY_BWT, config)); // initialize buffer of bwt
...
void construct_lcp_go(cache_config& config)
{
   ...
   // phase 1: calculate lcp_sml;  memory consumption: 2n bytes (lcp_sml=n bytes, text=n bytes)
   {
       ...
       util::store_to_cache(lcp_sml, "lcp_sml", config );
   }
#ifdef STUDY_INFORMATIONS
...
//			std::cout<<"# begin calculating LF' values"<<std::endl;
       int_vector<> lcp_big(nn, 0, bits::hi(n-1)+1); // lcp_big first contains adapted LF values and finally the big LCP values
       {
           ...
           bit_vector todo(n,0);  // bit_vector todo indicates which values are >= m in lcp_sml
           {
               ...
               int_vector_file_buffer<8> lcp_sml_buf(util::cache_file_name("lcp_sml", config)); // load lcp_sml
           ...
           int_vector_file_buffer<8> bwt_buf(util::cache_file_name(constants::KEY_BWT, config)); // load BWT
...
//							old_i = i;
       ...
       util::store_to_cache(lcp_big, "lcp_big", config );
   } // end phase 2
...
//		std::cout<<"# merge lcp_sml and lcp_big"<<std::endl;
   // phase 3: merge lcp_sml and lcp_big and save to disk
   {
       ...
       int_vector_file_buffer<> lcp_big_buf(util::cache_file_name("lcp_big", config)); 				// file buffer containing the big LCP values
       int_vector_file_buffer<8> lcp_sml_buf(util::cache_file_name("lcp_sml", config), buffer_size);// file buffer containing the small LCP values

       std::ofstream lcp_out_buf(util::cache_file_name(constants::KEY_LCP, config).c_str(), std::ios::binary | std::ios::trunc | std::ios::out);    // open out file stream
...
#endif
   int_vector<8> text; 
   util::load_from_cache(text, constants::KEY_TEXT, config);  // load text from file system
   int_vector_file_buffer<> sa_buf(util::cache_file_name(constants::KEY_SA, config));   // initialize buffer for suffix array
   const size_type n = sa_buf.int_vector_size;
   const size_type m = 254; // LCP[i] == m+1 corresp. to LCP[i]>= m+1; LCP[i] <= m corresp. to LCP[i] was calculated
   ...
   // phase 2: calculate lcp_big with PHI algorithm on remaining entries of LCP
   {
       int_vector<> lcp_big(0, 0, bits::hi(n-1)+1);//nn, 0, bits::hi(n-1)+1);
       {
           ...
           bit_vector todo(n,0);  // bit_vector todo indicates which values are > m in lcp_sml
           {
               // initialize bit_vector todo
               int_vector_file_buffer<8> lcp_sml_buf(util::cache_file_name("lcp_sml", config)); // load lcp_sml
               int_vector_file_buffer<> sa_buf(util::cache_file_name(constants::KEY_SA, config)); // load sa
               sa_buf.load_next_block();
               for (size_type i=0, r_sum=0, r = lcp_sml_buf.load_next_block(); r_sum < n;) {
                   if (r > 0)
                       sa_n_1 = sa_buf[r-1];
...
//			util::load_from_file(lcp_sml2, (dir+"lcp_sml_go").c_str() );
       ...
       alphabet[sigma] = '\0';
       {
           std::ofstream lcp_sml_out_buf(util::cache_file_name("lcp_sml", config).c_str(), std::ios::binary | std::ios::trunc | std::ios::out);    // open out file stream
           ...
           for (size_type i=0; i<sigma; ++i) {
               lcps[i].init(config.dir, alphabet[i]);
           }
...
//								l = lcp_sml2[lf] ? lcp_sml2[lf]-1 : 0; // l = LCP[LF[i]]-1; l < m+1
           ...
           util::register_cache_file("lcp_sml", config);
       }
		util::clear(text);
       if (n > 1000 and nn > 5*(n/6)) {  // if we would occupy more space than the PHI algorithm => switch to PHI algorithm
           construct_lcp_PHI<8>(config);
