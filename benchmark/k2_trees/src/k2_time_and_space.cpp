...
template<class t_kt>
size_type test_adj(const t_kt tree, idx_type node, size_type neighbors)
{
	idx_type cnt = 0;
	for(idx_type i = 0; i < neighbors; i++)
		if(tree.adj(node, i))
				cnt++;
	return cnt;
}
...
//! Construct a k2 tree from a file
/*! From an arc file, construct a k2_tree file.
 * \param file_path Path to the arc file defining the tree.
 * \returns A tree built from the file.
 */
template<class k2_type>
k2_type build_from_file(std::string file_path)
{
	...
	vector<t_tuple> e;
	typename k2_type::size_type max = 0;

	while(std::getline(infile, line)) {
		std::istringstream iss(line);
		idx_type v, u;
		if(!(iss >> v >> u)) {
			throw invalid_argument("Not expected line at construct");
		}
		if(v > max)
			max = v;
		if(u > max)
			max = u;
		e.push_back(t_tuple {v, u});
	}

	return k2_type(e, max + 1);
}

template<class k2_type>
void load_edges_from_file(const std::string &file_path,
						  vector<std::tuple<typename k2_type::idx_type,
...
int main(int argc, char* argv[])
{
    ...
    cerr << "JOla " << endl;
    if (argc < 4) {
        cout << "Usage: file num_bytes testcase_name k2_tree_id" << endl;
        return 1;
    }
    ...
    */
    ...
    cerr << "4" << endl;
    for (auto q: v)
    	cerr << "vector:" << std::get<0>(q) << "," << std::get<1>(q) << endl;
    cerr << max << endl;
    K2_TYPE k2(v, max);
    cerr << "5" << endl;
    auto stop = timer::now();
