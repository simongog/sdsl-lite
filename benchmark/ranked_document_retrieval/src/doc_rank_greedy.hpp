...
template <class t_csa = csa_wt<wt_huff<rrr_vector<63>>, 1000000, 1000000>, class t_wtd = wt_int<bit_vector_il<1024>>,
         class t_sadadfbv = rrr_vector<63>, class t_sadadf_select = typename t_sadadfbv::select_1_type,
...
int_vector<> m_doc_inverse_perm;    // inverse permutation
int_vector<> m_doc_weights;         // document weights
dfbv_type m_sada_df;                // sada df bv
dfbv_select_type m_sada_df_select;  // select on sada df bv
...
doc_rank_greedy(const std::string& input_file, sdsl::cache_config& cconfig, uint8_t num_bytes) {
    ...
    if (!cache_file_exists(KEY_SADADF + std::string("_") + util::class_to_hash(m_sada_df), cconfig)) {
        ...
        store_to_file(m_sada_df,
                      cache_file_name(KEY_SADADF + std::string("_") + util::class_to_hash(m_sada_df), cconfig));
        store_to_file(
            m_sada_df_select,
            cache_file_name(KEY_SADADFSEL + std::string("_") + util::class_to_hash(m_sada_df_select), cconfig));
    } else {
        load_from_cache(m_sada_df, KEY_SADADF + std::string("_") + util::class_to_hash(m_sada_df), cconfig);
        load_from_cache(m_sada_df_select, KEY_SADADFSEL + std::string("_") + util::class_to_hash(m_sada_df_select),
                        cconfig);
...
size_type serialize(std::ostream& out, structure_tree_node* v = NULL, std::string name = "") const {
    ...
    written_bytes += m_doc_inverse_perm.serialize(out, child, "doc_inverse_perm");
    written_bytes += m_doc_weights.serialize(out, child, "doc_weights");
...
void load(std::istream& in) {
    ...
    m_wtd.load(in);
    m_doc_perm.load(in);
    m_doc_inverse_perm.load(in);
    m_doc_weights.load(in);
    m_sada_df.load(in);
...
void swap(doc_rank_greedy& dr) {
    if (this != &dr) {
        ...
        m_doc_inverse_perm.swap(dr.m_doc_inverse_perm);
        m_doc_weights.swap(dr.m_doc_weights);
        m_sada_df.swap(dr.m_sada_df);
        util::swap_support(m_sada_df_select, dr.m_sada_df_select, &m_sada_df, &(dr.m_sada_df));
    }
}

//! Search for the k documents most "similar" to the query
...
for (const auto& p : query) {
        ...
        if (0 != backward_search(m_csa_full, 0, m_csa_full.size() - 1, p.begin(), p.end(), sp, ep)) {
            res.range_size += (ep - sp + 1);
            size_t f_t = calculate_df(sp, ep);
            ranges.emplace_back(sp, ep, m_doc_cnt, f_t);
        }
    }
    if (ranges.size()) {
        auto temp_res = m_wtd.topk_greedy_similarity(ranges, k);
        // remap doc ids
for (auto& doc : temp_res) {
            res.emplace_back(m_doc_inverse_perm[std::get<0>(doc)],std::get<1>(doc));
        }
    }
    return res;
}
...
void construct_D_array(sdsl::cache_config& cconfig, bit_vector::rank_1_type& doc_border_rank,
...
void construct_doc_permutation(sdsl::cache_config& cconfig) {
    size_t cur_id = 0;
    using u64p_t = std::pair<uint64_t, uint64_t>;
    std::vector<u64p_t> weights;

    // check if docweight file was specified
    auto itr = cconfig.file_map.find(KEY_DOCWEIGHT);
    if (itr == cconfig.file_map.end()) {
        // not found. compute from text
        const char* KEY_TEXT = key_text_trait<WIDTH>::KEY_TEXT;
        std::string text_file = cache_file_name(KEY_TEXT, cconfig);
        int_vector_buffer<WIDTH> text_buf(text_file);
        size_t doc_size = 0;
        for (size_type i = 0; i < text_buf.size(); ++i) {
            if (t_doc_delim == text_buf[i]) {
                weights.emplace_back(cur_id, doc_size);
                doc_size = 0;
                cur_id++;
            }
            doc_size++;
        }
        std::cout << "found " << weights.size() << " documents" << std::endl;
    } else {
        // found. -> parse
        ...
        for (; dw_in.getline(buf, 512);) {
            weights.emplace_back(cur_id, std::strtoull(buf, NULL, 10));
            cur_id++;
        }
        std::cout << "found " << weights.size() << " documents" << std::endl;
    }
    ...
    // sort by weight
    auto ws = [](const u64p_t & a, const u64p_t & b) { return a.second < b.second; }
    ;
    std::sort(weights.begin(), weights.end(), ws);

    m_doc_perm.width(bits::hi(weights.size()) + 1);
    m_doc_perm.resize(weights.size());
    m_doc_inverse_perm.width(bits::hi(weights.size()) + 1);
    m_doc_inverse_perm.resize(weights.size());
    m_doc_weights.resize(weights.size());
    for (size_t i = 0; i < weights.size(); i++) {
        m_doc_perm[weights[i].first] = i;
        m_doc_inverse_perm[i] = weights[i].first;
        m_doc_weights[i] = weights[i].second;
    }
    util::bit_compress(m_doc_weights);
}

void construct_sada_df(sdsl::cache_config& cconfig) {
    // construct cst
    ...
    if (!cache_file_exists(KEY_TMPCST, cconfig)) {
        ...
        store_to_file(temp_cst, cache_file_name(KEY_TMPCST, cconfig));
    } else {
        load_from_file(temp_cst, cache_file_name(KEY_TMPCST, cconfig));
    }

    wt_int<> wtc;
    std::string d_file = cache_file_name(KEY_DARRAY, cconfig);
    int_vector_buffer<> D(d_file);
    if (!cache_file_exists(KEY_WTC, cconfig)) {
        {
        ...
        store_to_file(wtc, cache_file_name(KEY_WTC, cconfig));
    } else {
        load_from_file(wtc, cache_file_name(KEY_WTC, cconfig));
    }
...
size_type calculate_df(size_t sp, size_t ep) const {
    size_t dup = 0;
