...
mm_dir=args[1]
source(paste(mm_dir,"/../../benchmark/basic_functions.R", sep=""))
configFile=paste(mm_dir,"/mm-log.config",sep="")
...
if ( tikz_output ){ 
	...
	tikz("mm-log.tex", width = 3.2, height = 1.3, standAlone = F)
	par(oma=c(0,0,0,0))
...
config <- readConfig(configFile,c("ID","start","end","fill-color","stroke-color","NAME"))
...
for( x in 1:nrow(config) ){
    ...
    if ( length(t1) > 0 ){
		for ( j in 1:length(t1) ){
			interval <- subset(t, t[1] >= t1[j] & t[1] <= t2[j])
			xs <- interval[[1]]
			ys <- interval[[2]]
