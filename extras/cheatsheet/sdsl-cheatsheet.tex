...
\subsection{Compressed Unsigned Integer Vectors}
...
\subsection{Select Supports (\href{\sdslgitinc/select_support.hpp}{SS})}\label{sec-SS}
...
\subsection{Wavelet Trees (\href{\sdslgitinc/wavelet_trees.hpp}{WT})}
Mention: alphabet strategies
\begin{tabular}{@{}ll@{}}
\textit{Class}    &\textit{Description} \\
\href{\sdslgitinc/wt_huff.hpp}{\sdslwthuff} &
Huffman shaped wavelet tree for byte alphabets.\\
\href{\sdslgitinc/wt_int.hpp}{\sdslwtint} &
Balanced wavelet tree for integer alphabets.\\
\href{\sdslgitinc/wt.hpp}{\sdslwt} &
Balanced wavelet tree for byte alphabets.\\
\href{\sdslgitinc/wt_rlmn.hpp}{\sdslwtrlmn} &
...
\subsection{Compressed Suffix Arrays (\href{\sdslgitinc/suffix_arrays.hpp}{CSA})}
\begin{tabular}{@{}ll@{}}
\textit{Class}    &\textit{Description} \\
...
(e.g.
\href{\sdslgitinc/csa_alphabet_strategy}{\sdslbytealphabetstrategy},
\href{\sdslgitinc/csa_alphabet_strategy}{\sdslsuccinctbytealphabetstrategy},
\href{\sdslgitinc/csa_alphabet_strategy}{\sdslintalphabetstrategy})
and SA sampling strategy 
(e.g. 
 \href{\sdslgitinc/csa_sampling_strategt}{\sdslsaordersasampling},
 \href{\sdslgitinc/csa_sampling_strategt}{\sdsltextordersasampling}
)
...
\subsection{Compressed Suffix Trees (\href{\sdslgitinc/suffix_trees.hpp}{CST})}
...
\begin{tabular}{@{}p{0.9\linewidth}@{}}
\code{load\_from\_file(x, file)} \\
Load an \sdsl\ object \code{o}, which is stored in \code{file}. \\
\code{o.load(in)} \\
Read an \sdsl\ object \code{o} from \code{std::istream}-object \code{in}.\\
\code{load\_vector} \\
\end{tabular}
TODO: mention io wrappers
...
\begin{tabular}{@{}p{0.9\linewidth}@{}}
\code{store\_to\_file(x, file)} \\
Store an \sdsl\ object \code{o} to \code{file}. \\
\code{o.serialize(out)}\\
Write \sdsl\ object \code{o} to \code{std::ostream}-object \code{out}.\\
\code{serialize\_vector}	  \\
\end{tabular}

\code{size\_in\_bytes(o)}
\code{write\_member($x$, out)}
\code{read\_member($x$, in)}
\code{write\_structure<JSON\_FORMAT>(o, out)} 
...
\code{demangle(x)}   & Demangles output of \code{typeid(x).name()}.\\
\code{demangle2(x)}  & Simplifies output of \code{demangle}. E.g. removes \code{sdsl::}-prefixes, \ldots \\
...
\code{cache\_config(del\_file, tmp\_dir, id, file\_map)}
\code{cache\_file\_name(key, config)}
\code{register\_cache\_file(key, config)}
\code{cache\_file\_exists(key, config)}
\code{load\_from\_cache(t, key, config)}
\code{store\_to\_cache(t, key, config)}
...
\section{Benchmarks}
Configurable benchmarks (see .config-files) 
for operations \code{count}, \code{locate},
and \code{extract} are in the subdirectory
\code{benachmark}.
