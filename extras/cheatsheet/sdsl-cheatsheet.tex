...
The header which is required to used the data structure
of each subsection is linked in the heading.
...
RSs add rank functionality to BV. Method
...
Huffman WT for byte alphabets.\\
\href{\sdslgitinc/wt_int.hpp}{\sdslwtint} &
Balanced WT for integer alphabets.\\
\href{\sdslgitinc/wt.hpp}{\sdslwt} &
Balanced WT for byte alphabets.\\
\href{\sdslgitinc/wt_rlmn.hpp}{\sdslwtrlmn} &
Run-length compressed WT. 
...
the suffix arrays (SA) and its inverse (ISA). Note that 
\begin{tabular}{@{}ll@{}}
\textit{Class}    &\textit{Description} \\
...
We represented a parentheses sequence as \code{bit\_vector}. 
...
\section{Constructing data structures (1)}
...
\end{tabular}
Object can also be  \code{construct\_im(o,data,num\_bytes=0)}
...
In general \code{o} can also be serialized
into a \code{std::ostream}-object \code{out}
with \code{o.serialize(o)}.
...
In general \code{o.load(in)} can load \code{o} from
a \code{std::istream}-object \code{in}.
...
\section{Constructing data structures (2)}
\code{construct\_im(o,file,num\_bytes=0)}\\
...
\section{Measuring Time \& Space}
Create an instance \code{sw} of class \code{util::stop\_watch}
to measure time. Call \code{sw.start()} to start
the timing and \code{sw.stop()} to stop it. Methods
\code{user\_time()}, \code{sys\_time()}, and
\code{real\_time()} return user, system and
elapse time in milliseconds. Use  \code{sw.stop()} and
\code{abs\_user\_time()}, \code{abs\_sys\_time()}, and \code{abs\_real\_time()}
to get the times since the creation of the process.
