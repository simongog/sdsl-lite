...
//! Document this class
/*!
 * More documentation for the class. E.g.
 *   - where was it described in literature
 *   - key operations
 *   - space and time complexities
 *
 * A select_support on 1-bits is needed for the child(v,i) operation.
 * A select_support on 0-bits is needed for the parent operation.
 *
 * Disadvantages of louds: No efficient support for subtree size.
*/
template<class BitVector, class SelectSupport1, class SelectSupport0>
class louds_tree
{
    public:
        typedef bit_vector::size_type 	size_type;
    ...
    private:
        ...
        size_type		m_nodes; 		// number of nodes of the tree
    public:

        //! Constructor
        template<class BFSIterator>
        louds_tree(const BFSIterator& it) {
            bit_vector bv;
        ...
        bool is_leaf(const node_type& v) const {
            // node is the last leaf        or  has no children, so m_bp[v.pos]==1
            return (v.pos+1 == m_bv.size()) or m_bp[v.pos+1];
        }
         ...
         *  \param v A node.
         */
        node_type degree(const node_type& v) const {
