...
*/
/*! \file enc_vector_prac2.hpp
   \brief enc_vector_prac2.hpp contains the sdsl::enc_vector_prac2 class.
   \author Simon Gog
*/  
...
#ifndef SDSL_ENC_VECTOR_PRAC2
#define SDSL_ENC_VECTOR_PRAC2
...
/*! The values of a enc_vector_prac2 are immutable after the constructor call. The class
...
class enc_vector_prac2{
	...
	typedef random_access_const_iterator<enc_vector_prac2> iterator;// STL Container requirement
	...
	typedef Coder								coder;
	typedef typename enc_vector_prac2_trait<fixedIntWidth>::int_vector_type int_vector_type;
	...
	void copy(const enc_vector_prac2 &v);
	...
	public:
		...
		//! Copy constructor
		/*! \param v The enc_vector_prac2 to copy.
		  	Required for the Assignable Concept of the STL
		 */
		enc_vector_prac2(const enc_vector_prac2 &v);
		...
		enc_vector_prac2(const Container &c){
			construct();
			init(c);
		};
		...
		*/	
		...
		enc_vector_prac2(int_vector_file_buffer<int_width, size_type_class> &v_buf){
			construct();
			init(v_buf);
		}
		...
		//! The number of elements in the enc_vector_prac2.
		/*!
		...
		//!	Returns if the enc_vector_prac2 is empty.
		...
		//! Swap method for enc_vector_prac2
		/*! The swap method can be defined in terms of assignment.
			...
			\param v enc_vector_prac2 to swap.

			Required for the Assignable Conecpt of the STL.
		  */
		void swap(enc_vector_prac2 &v);

		//! Iterator that points to the first element of the enc_vector_prac2.
		/*!
		 * 	Required for the Container Concept of the STL.
		 *  \sa end()
		 */
		...
		//! Iterator that points to the position after the last element of the enc_vector_prac2.
		/*!
		 *	Required for the Container Concept of the STL
		 *  \sa begin()
	     */
		...
		// Iterator that points to the last element of the enc_vector_prac2.
		/*
		 * 	Required for the Container Concept of the STL.
		 *  \sa rend()
		 */
//		reverse_iterator rbegin()const;
		...
		// Iterator that points to the position before the first element of the enc_vector_prac2.
		/*
		 *	Required for the Container Concept of the STL
		 *  \sa rbegin()
	     */
//		reverse_iterator rend()const;
		 ...
		 *	Required for the Assignable Concept of the STL.
		 */
		enc_vector_prac2& operator=(const enc_vector_prac2 &v);

		//! Equality Operator
		/*! Two enc_vector_prac2s are equal if all member variables are equal
		 *  (including the sample density of the enc_vector_prac2s).
		 *  \note If the sample density is not equal you should use
		 *  SDSAlgorithm::equal_container_values to compare two enc_vector_prac2s.
		 *
		 * 	Required for the Equality Comparable Concept of the STL.
		 *  \sa operator!=
		 */
		bool operator==(const enc_vector_prac2 &v)const;

		//! Unequality Operator
		/*! Two enc_vector_prac2s are unuequal if not all member variables are equal
		 *  (including the sample density of the enc_vector_prac2s).
		 *  \note If the sample density is not equal you should use
		 *  SDSAlgorithm::equal_container_values to compare two enc_vector_prac2s.
		 *
		...
		//! Serialzes the enc_vector_prac2 to a stream.
		/*! \param out Outstream to write the data structure.
		    \return The number of written bytes.
		 */
		size_type serialize(std::ostream &out) const;

		//! Load the enc_vector_prac2 from a stream.
		void load(std::istream &in);
	
		//! Returns the ith sample of enc_vector_prac2
...
inline uint32_t enc_vector_prac2<Coder, SampleDens, fixedIntWidth>::get_sample_dens() const{
	if(SampleDens == 0)
...
template<class Coder, uint32_t SampleDens, uint8_t fixedIntWidth>
inline void enc_vector_prac2<Coder, SampleDens, fixedIntWidth>::set_sample_dens(const uint32_t sample_dens){
	m_sample_dens = sample_dens;
}

template<class Coder, uint32_t SampleDens, uint8_t fixedIntWidth>
inline typename enc_vector_prac2<Coder, SampleDens,fixedIntWidth>::value_type enc_vector_prac2<Coder, SampleDens,fixedIntWidth>::operator[](const size_type i)const {
	assert( i+1 != 0 );
#ifdef SDSL_DEBUG	
	if( i >= m_elements  ){
		throw std::out_of_range("OUT_OF_RANGE_ERROR: enc_vector_prac2::operator[](size_type); i >= size()!");
		return 0;
	}
#endif	
...
template<class Coder, uint32_t SampleDens, uint8_t fixedIntWidth>
inline typename enc_vector_prac2<Coder, SampleDens,fixedIntWidth>::value_type enc_vector_prac2<Coder, SampleDens,fixedIntWidth>::sample(const size_type i)const {
	assert( i*get_sample_dens()+1 != 0 );
#ifdef SDSL_DEBUG	
	if( i*get_sample_dens() >= m_elements  ){
		throw std::out_of_range("OUT_OF_RANGE_ERROR: enc_vector_prac2::sample(size_type); i*get_sample_dens() >= size()!");
		return 0;
	}
#endif	
...
template<class Coder, uint32_t SampleDens, uint8_t fixedIntWidth>
inline enc_vector_prac2<>::size_type enc_vector_prac2<Coder, SampleDens,fixedIntWidth>::size()const
{
	return m_elements;
}

template<class Coder, uint32_t SampleDens, uint8_t fixedIntWidth>
inline enc_vector_prac2<>::size_type enc_vector_prac2<Coder, SampleDens,fixedIntWidth>::max_size()
{
	return int_vector<>::max_size()/2; // each element could possible occupy double space with selfdelimiting codes
}

template<class Coder, uint32_t SampleDens, uint8_t fixedIntWidth>
inline bool enc_vector_prac2<Coder, SampleDens,fixedIntWidth>::empty()const
{
	return 0==m_elements;
}
...
template<class Coder, uint32_t SampleDens, uint8_t fixedIntWidth>
void enc_vector_prac2<Coder, SampleDens,fixedIntWidth>::copy(const enc_vector_prac2<Coder, SampleDens,fixedIntWidth> &v){
...
template<class Coder, uint32_t SampleDens, uint8_t fixedIntWidth>
enc_vector_prac2<Coder, SampleDens,fixedIntWidth>::enc_vector_prac2(const enc_vector_prac2 &v){
	copy(v);
}

template<class Coder, uint32_t SampleDens, uint8_t fixedIntWidth>
enc_vector_prac2<Coder, SampleDens,fixedIntWidth>& enc_vector_prac2<Coder, SampleDens,fixedIntWidth>::operator=(const enc_vector_prac2<Coder, SampleDens,fixedIntWidth> &v){
...
template<class Coder, uint32_t SampleDens, uint8_t fixedIntWidth>
bool enc_vector_prac2<Coder, SampleDens,fixedIntWidth>::operator==(const enc_vector_prac2<Coder, SampleDens,fixedIntWidth> &v)const{
	if(this == &v)
		return true;
...
template<class Coder, uint32_t SampleDens, uint8_t fixedIntWidth>
void enc_vector_prac2<Coder, SampleDens,fixedIntWidth>::swap(enc_vector_prac2<Coder, SampleDens,fixedIntWidth> &v){
	if(this != &v){// if v and _this_ are not the same object
...
template<class Coder, uint32_t SampleDens, uint8_t fixedIntWidth>
template<class Container>
void enc_vector_prac2<Coder, SampleDens,fixedIntWidth>::init(const Container &c){
	// clear bit_vectors
	clear();
		...
		for(it = c.begin(); it != end; ++it, --no_sample){
			...
			if(!no_sample){ // add a sample
				...
				*sv_it = z_size; ++sv_it;
				...
				x = v2-v1;
				if(v2 == v1){
					throw std::logic_error("enc_vector_prac2 cannot decode adjacent equal values!");
				}
...
template<class Coder, uint32_t SampleDens, uint8_t fixedIntWidth>
template<uint8_t int_width, class size_type_class>
void enc_vector_prac2<Coder, SampleDens,fixedIntWidth>::init(int_vector_file_buffer<int_width, size_type_class> &v_buf){
	// clear bit_vectors
	clear();
	...
	size_type z_size2 = 0, big_val_cnt=0, old_z_size=0, old_i=0;
	size_type big_val[sd+1], big_val_pos[sd+1];
	uint8_t log_sd=bit_magic::l1BP(sd)+1;
	size_type saved_bits=0;
//  (1) Calculate maximal value of samples and of deltas
	for(size_type i=0, r_sum=0, r = v_buf.load_next_block(), no_sample = 0; r_sum < n; ){
		for(; i < r_sum+r; ++i, --no_sample){
			...
			if( !no_sample ){ // is sample
				...
				size_type bits=0;
				bits += log_sd; // bits for big_val_cnt
				for(size_type j=0; j<big_val_cnt;++j){
					bits += Coder::encoding_length( big_val[j] );
					if( j == 0 )
						bits += Coder::encoding_length( big_val_pos[j]+1 );
					else
						bits += Coder::encoding_length( big_val_pos[j]-big_val_pos[j-1] );
				}
				big_val_cnt = 0;
				if( z_size - old_z_size > bits ){
					z_size2 += bits;
				}else{
					z_size2 += (z_size-old_z_size);
				}
				old_z_size = z_size;
				old_i = i;
			}else{
				if( v2 == v1){ throw std::logic_error("enc_vector_prac2 cannot decode adjacent equal values!"); }
				z_size += Coder::encoding_length( v2-v1 );
				if( v2-v1 > 1){
					big_val[big_val_cnt] = v2-v1;
					big_val_pos[big_val_cnt] = i-old_i;
					++big_val_cnt;
				}
			}
			v1 = v2;
	...
	std::cerr<<"z in MB = " << z_size/(8.0*1024.0*1024.0) << std::endl;
	std::cerr<<"z2 in MB = " << z_size2/(8.0*1024.0*1024.0) << std::endl;
	std::cerr<<"saving in MB = " << (z_size-z_size2)/(8.0*1024.0*1024.0) << std::endl;
...
template<class Coder, uint32_t SampleDens, uint8_t fixedIntWidth>
enc_vector_prac2<>::size_type enc_vector_prac2<Coder, SampleDens,fixedIntWidth>::serialize(std::ostream &out) const{
...
template<class Coder, uint32_t SampleDens, uint8_t fixedIntWidth>
void enc_vector_prac2<Coder, SampleDens,fixedIntWidth>::load(std::istream &in){
...
template<class Coder, uint32_t SampleDens, uint8_t fixedIntWidth>
const typename enc_vector_prac2<Coder,SampleDens,fixedIntWidth>::const_iterator enc_vector_prac2<Coder, SampleDens,fixedIntWidth>::begin()const{
	return const_iterator(this, 0);
}

template<class Coder, uint32_t SampleDens, uint8_t fixedIntWidth>
const typename enc_vector_prac2<Coder,SampleDens,fixedIntWidth>::const_iterator enc_vector_prac2<Coder, SampleDens,fixedIntWidth>::end()const{
