...
template<uint32_t SampleDens=32, uint8_t fixedIntWidth=0>
class _sa_order_sampling_strategy : public int_vector<fixedIntWidth>{
	public:
		typedef int_vector<fixedIntWidth> base_type;
		typedef typename base_type::size_type  size_type;	// make typedefs of base_type visible
		typedef typename base_type::value_type value_type;	//
		enum{ sample_dens = SampleDens };
...
class sa_order_sa_sampling{
	public:
		...
		class type{         // sampling strategy class with the Sampling density of the CSA
			public:
			typedef _sa_order_sampling_strategy<Csa::sa_sample_dens, fixedIntWidth> sample_type;
		};	
};


template<uint32_t SampleDens=32, 
	     class bit_vector_type=bit_vector, 
		 class rank_support_type=typename bit_vector_type::rank_1_type,
		 uint8_t fixedIntWidth=0 
		 >
class _text_order_sampling_strategy : public int_vector<fixedIntWidth>{
	private:
...
template<class bit_vector_type=bit_vector, class rank_support_type=typename bit_vector_type::rank_1_type, uint8_t fixedIntWidth=0>
class text_order_sa_sampling{
	public:
		template<class Csa> // template inner class which is used in CSAs to parametrize the
		class type{         // sampling strategy class with the Sampling density of the CSA
			public:
			typedef _text_order_sampling_strategy<Csa::sa_sample_dens, 
			                                      bit_vector_type,
												  rank_support_type,
												  fixedIntWidth
												  > sample_type;
