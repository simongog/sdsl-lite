...
/* 
 */	
int_vector<32> get_rnd_positions(uint8_t log_s, uint64_t &mask, uint64_t m=0, uint64_t x=17);

template<class Vector>
void test_int_vector_random_access(const Vector &v, bit_vector::size_type times=100000000){
	typedef bit_vector::size_type size_type;
	uint64_t mask;
	int_vector<32> rands = get_rnd_positions(20, mask, v.size());
	size_type cnt=0;
...
void test_rank_random_access(const Rank &rank, bit_vector::size_type times=20000000){
	typedef bit_vector::size_type size_type;
	uint64_t mask;
	int_vector<32> rands = get_rnd_positions(20, mask, rank.size()+1);
	size_type cnt=0;
...
void test_csa_access(const Csa &csa, typename Csa::size_type times=1000000){
	...
	size_type cnt=0;
	uint64_t mask;
	int_vector<32> rands = get_rnd_positions(20, mask, csa.size());
...
void test_pattern_matching(const Csa &csa, 
	...
	typedef typename Csa::size_type size_type;
	uint64_t mask;
	int_vector<32> rands = get_rnd_positions(15, mask, csa.size()-pattern_len);
...
void test_rank_bwt_access(const Csa &csa, typename Csa::size_type times=1000000){
	...
	uint64_t mask, s=20;
	int_vector<32> rands = get_rnd_positions(s, mask, csa.size()+1);
...
void test_select_bwt_access(const Csa &csa, typename Csa::size_type times=500000){
	...
	// precalc queries, distribution of chars is important! 
	uint64_t mask, s=20;
	int_vector<32> rands = get_rnd_positions(s, mask, csa.size());
	unsigned char * c_rands = new unsigned char[1<<s];
	for(size_type i=0; i<rands.size(); ++i){
		c_rands[i] = csa.bwt[ rands[i] ];
...
void test_lcp_random_access(Lcp &lcp, typename Lcp::size_type times=10000000){
	...
	if( times > n )
		times = n;
	uint64_t mask;
	int_vector<32> rands = get_rnd_positions(20, mask, n);
	size_type cnt=0;
 ...
 */
template<class Lcp>
void test_lcp_random_sequential_access(Lcp &lcp, typename Lcp::size_type times=1000000, typename Lcp::size_type seq_len=32){
	typedef typename Lcp::size_type size_type;
	size_type n = lcp.size();
	if( times > n )
		times = n;
	...
	int_vector<32> rands(1<<s ,0);
	util::set_random_bits(rands, 17);
	for(int_vector<32>::size_type i=0; i < rands.size(); ++i){
		rands[i] = rands[i] % n;
