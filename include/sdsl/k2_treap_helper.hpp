...
struct node_type {
    ...
    //create bitmask from position a to b
    static inline short createMask(short a, short b)
    {
        unsigned r = 0;
        for (short i=a; i<=b; i++)
            r |= 1 << i;
     ...
     * Interleaves the
     */
    static inline uint interleaveLastBits(ushort x, ushort y, int h, int l) {
        short mask = createMask(0, l - h);
        x = x &  mask;
        y = y &  mask;
