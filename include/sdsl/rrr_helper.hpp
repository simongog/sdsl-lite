...
class binomial_coefficients{
	...
	static class impl{
		...
		impl(){
			for(uint16_t k=0; k <= MAX_SIZE; ++k){ table[k][k] = 1; space[k][k] = 0; } // initialize diagonal 
			for(uint16_t k=0; k <= MAX_SIZE; ++k){ table[0][k] = 0; space[0][k] = 0; } // initialize first row
			for(uint16_t nn=0; nn <= MAX_SIZE; ++nn){ table[nn][0] = 1; space[nn][0] = 0; } // initialize first column 
