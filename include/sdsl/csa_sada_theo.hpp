...
//		}
	...
	public:
		 ...
		 *  \sa max_size
		 */
		size_type size()const{ return m_level > 0 ? m_b[0].size() : m_sa.size();}

		//! Returns the largest size that csa_sada_theo can ever have.
		/*! Required for the Container Concept of the STL.
		 *  \sa size
		 */
		static size_type max_size(){ return EncVector::max_size(); }
...
void csa_sada_theo<EncVector,RankSupport>::copy(const csa_sada_theo<EncVector,RankSupport> &csa){
	...
	if(m_b_rank != NULL){ delete [] m_b_rank; m_b_rank = NULL; }
	if(m_b		!= NULL){ delete [] m_b; m_b = NULL; }
	if(m_psi	!= NULL){ delete [] m_psi; m_psi = NULL; }
	...
	{ // copy arrays
		...
		for(size_type i=0; i < m_level; ++i){ m_psi[i]	= csa.m_psi[i]; }

		m_b		= new int_vector<1>[m_level];
		for(size_type i=0; i < m_level; ++i){ m_b[i]	= csa.m_b[i]; }
