...
class nn_dict_dynamic
{
    ...
    public:
        ...
        size_type next(const size_type idx)const
        {
            ...
            uint64_t d = m_depth;	// current depth of node
            ...
            node = m_tree[ v_node_position-m_offset[d] ]>>off;
            ...
            while (v_node_position < m_v_begin_leaves) {
                ...
                node = m_tree[ v_node_position-m_offset[d] ];
        ...
        size_type prev(const size_type idx)const
        {
            ...
            // Go up until a 1-bit is found
            node = m_tree[ v_node_position-m_offset[d] ]<<(63-off);
        ...
        class reference
        {
            ...
            public:
                ...
                reference& operator=(bool x)
                {
                    ...
                    uint64_t d = m_pbv->m_depth;	// current depth of node

                    v_node_position = m_pbv->m_v_begin_leaves + (m_idx>>6);
                    uint8_t offset = m_idx & 0x3F; // pos mod 64
                    if (x) {
                        while (true) {
                            r_node_position = v_node_position - m_pbv->m_offset[d];
                        ...
                        while (true) {
                            ...
                            if (!((w>>offset) & 1)) { // if the bit is already 0
                                ...
                                if (!m_pbv->m_tree[r_node_position] and d) { // go up in the tree
                                    --d; --v_node_position;
                                    offset = v_node_position&0x3F;
                                    v_node_position >>= 6;
                                } else {
