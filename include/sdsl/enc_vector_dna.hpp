...
//		assert(result == expected1);
		...
		do{
			++data; ++blocknr;
			sumblock >>= 8;
			if(blocknr==8){++data; sumblock=*(data+8); blocknr=0;}
		...
		for(it = c.begin(); it != end; ++it, --no_sample){
			...
			if(!no_sample){ // add a sample
				...
				if( newblocknr == 8 ){ // if the next integer will be written in the sumblock
					...
					// calculate sumblock
					if( writeblock == 7 ){ write_sumblock(x, writeblock, sbp, ovp); x=0; }
					for(int i=7;i>=0;--i){sumblockword<<=8; sumblockword+=sumblock[i];sumblock[i]=0;overflow[i]=false;}
					*z_data = sumblockword;
					++z_data;
