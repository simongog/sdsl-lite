...
struct node_type {
    ...
    static constexpr unsigned short MortonTable_3bitwise_512[512] =
            {
                    0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x80, 0x81,
                    0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0xc0, 0xc1, 0xc2, 0xc3, 0xc4, 0xc5, 0xc6, 0xc7, 0x100, 0x101,
                    0x102, 0x103, 0x104, 0x105, 0x106, 0x107, 0x140, 0x141, 0x142, 0x143, 0x144, 0x145, 0x146, 0x147,
                    0x180, 0x181, 0x182, 0x183, 0x184, 0x185, 0x186, 0x187, 0x1c0, 0x1c1, 0x1c2, 0x1c3, 0x1c4, 0x1c5,
    ...
    struct access_shortcut_helper<8> {
        ...
        */
        static inline uint64_t interleaveFirstBits(uint64_t x, uint64_t y, int h, int l) {
            ...
            z = z | //only look at top 7 bits and later at bottom 9 bits, table gives 15 bits, which should be 18 bits (3 zeros at the end missing), shift the bits 3 to the right to obtain the 18 bits, then shift the 18 bits to the right in the case of 32 Bit discarding the top two bits (3+16 = 19)
